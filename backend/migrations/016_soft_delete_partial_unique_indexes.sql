-- [sqlite-converted] from PostgreSQL migration: 016_soft_delete_partial_unique_indexes.sql
...
-- 删除旧的唯一约束（可能的命名方式）
-- [sqlite] DROP CONSTRAINT users_email_key on users → try DROP INDEX
...
-- 删除旧的唯一约束
-- [sqlite] DROP CONSTRAINT groups_name_key on groups → try DROP INDEX
...
-- 删除旧的唯一约束/索引
-- [sqlite] DROP CONSTRAINT user_subscriptions_user_id_group_id_key on user_subscriptions → try DROP INDEX
