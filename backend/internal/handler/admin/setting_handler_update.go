...
func (h *SettingHandler) UpdateSettings(c *gin.Context) {
	...
	hiddenMenuJSON := previousSettings.HiddenMenuKeys
	if req.HiddenMenuKeys != nil {
		keys := make([]string, 0, len(*req.HiddenMenuKeys))
		for _, k := range *req.HiddenMenuKeys {
		...
		if err != nil {
			response.BadRequest(c, "Invalid hidden_menu_keys")
			return
		}
		hiddenMenuJSON = string(b)
	}
	...
	payload := dto.SystemSettings{
		...
		HiddenMenuKeys:                                         dto.ParseHiddenMenuKeys(updatedSettings.HiddenMenuKeys),
