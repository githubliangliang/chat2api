...
func (r *groupRepository) ExistsByIDs(ctx context.Context, ids []int64) (map[int64]bool, error) {
	...
	inClause, args := sqlInt64In("id", uniqueIDs, 1)
	rows, err := r.sql.QueryContext(ctx, `
		SELECT id
		FROM groups
...
func (r *groupRepository) DeleteCascade(ctx context.Context, id int64) ([]int64, error) {
	...
	// SQLite serializes writers; this read participates in the surrounding transaction.
...
func (r *groupRepository) UpdateSortOrders(ctx context.Context, updates []service.GroupSortOrderUpdate) error {
	...
	inClause, inArgs := sqlInt64In("id", groupIDs, 1)
	if err := scanSingleRow(
		ctx,
		r.sql,
		`SELECT COUNT(*) FROM groups WHERE deleted_at IS NULL AND `+inClause,
		inArgs,
		&existingCount,
	); err != nil {
		return err
