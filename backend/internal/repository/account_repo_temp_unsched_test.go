...
func TestAccountRepository_GrokCredentialConditionalMutationsAreEligibleAndAtomicallyPropagated(t *testing.T) {
	...
	t.Run("permanent", func(t *testing.T) {
		...
		require.Contains(t, normalized, "a.temp_unschedulable_until IS NULL OR a.temp_unschedulable_until <= CURRENT_TIMESTAMP")
		require.Contains(t, normalized, "a.rate_limit_reset_at IS NULL OR a.rate_limit_reset_at <= CURRENT_TIMESTAMP")
		require.Contains(t, normalized, "a.overload_until IS NULL OR a.overload_until <= CURRENT_TIMESTAMP")
		require.Contains(t, normalized, "json(a.credentials) = json($7)")
		require.Contains(t, normalized, "a.proxy_id IS $8")
...
func TestAccountRepository_SetGrokOAuthRefreshErrorIfCredentialsUnchanged_UsesAttemptCredentialsAndProxy(t *testing.T) {
	...
	normalized := normalizeSQLWhitespace(exec.execQueries[0])
	require.Contains(t, normalized, "json(a.credentials) = json($7)")
	require.Contains(t, normalized, "proxy_id IS $8")
...
func TestAccountRepository_UpdateGrokOAuthCredentialsIfUnchanged_UsesExactAttemptStateAndAtomicOutbox(t *testing.T) {
	...
	normalized := normalizeSQLWhitespace(exec.execQueries[0])
	require.Contains(t, normalized, "WITH updated AS")
	require.Contains(t, normalized, "credentials = json($1)")
	require.Contains(t, normalized, "json(a.credentials) = json($5)")
	require.Contains(t, normalized, "proxy_id IS $6")
	require.Contains(t, normalized, "INSERT INTO scheduler_outbox")
...
func TestAccountRepository_ListOAuthRefreshCandidatePage_SQLFilter(t *testing.T) {
	...
	require.Contains(t, normalized, "json_extract(credentials, '$.refresh_token') IS NOT NULL")
	require.Contains(t, normalized, "trim(json_extract(credentials, '$.refresh_token')) <> ''")
	require.Contains(t, normalized, "temp_unschedulable_until > CURRENT_TIMESTAMP")
	...
	require.Equal(t, int64(100), capturedArgs[5])
	require.Equal(t, 200, capturedArgs[6])
	require.Contains(t, capturedArgs[:5], service.PlatformGrok)
