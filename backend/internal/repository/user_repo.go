...
func (r *userRepository) GetLatestUsedAtByUserIDs(ctx context.Context, userIDs []int64) (map[int64]*time.Time, error) {
	...
	inClause, args := sqlInt64In("user_id", userIDs, 1)
	query := `
...
func (r *userRepository) SetBalance(ctx context.Context, id int64, value float64) (service.BalanceChange, error) {
	...
	// SQLite does not support PostgreSQL's UPDATE ... FROM ... RETURNING prev.col pattern.
	if r.client != nil && r.client.Driver().Dialect() == dialect.SQLite {
		old, err := r.currentBalance(ctx, id)
		if err != nil {
			return service.BalanceChange{}, err
		}
		res, err := clientFromContext(ctx, r.client).ExecContext(ctx,
			`UPDATE users SET balance = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 AND deleted_at IS NULL`,
			value, id)
		if err != nil {
			return service.BalanceChange{}, err
		}
