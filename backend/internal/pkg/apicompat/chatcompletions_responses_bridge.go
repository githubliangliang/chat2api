...
type ResponsesToChatOptions struct {
	...
	// compaction); a thinking-mode upstream rejects such histories with 400
...
func buildChatMessagesFromItems(messages []ChatMessage, rawItems []json.RawMessage, opts *ResponsesToChatOptions) ([]ChatMessage, toolOutputMediaByCallID, error) {
	...
	// surviving tool outputs. A thinking-mode upstream emits reasoning only once
	// per turn, so chained tool calls (reasoning → call A → output A → call B)
	// leave call B's assistant message without reasoning_content and the upstream
	// 400s the history; replaying the turn's reasoning on B's message satisfies
	// the contract. Only a user-side item ends the turn and clears it.
	...
	for _, raw := range rawItems {
		...
		// Thinking mode requires the reasoning_content from a prior
