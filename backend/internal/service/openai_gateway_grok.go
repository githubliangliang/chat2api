...
func normalizeGrokReasoningEffortValue(raw, model string) (string, bool) {
	...
	case "xhigh", "extrahigh":
		...
		return "high", true
	case "max", "ultra":
		// max/ultra 是别名而非 xai 档位，无条件映射到 high。
		return "high", true
	default:
		return "", false
	}
...
func stripRedundantGrokViewImageTool(body []byte) ([]byte, error) {
	...
	// An explicit tool_choice for view_image is the caller insisting; leave it.
	...
	// tool_choice:"required" with an empty tool list is a rejected request.
