...
// ---------------------------------------------------------------------------
// Group per-model pricing (Group → Channel → 内置)
// ---------------------------------------------------------------------------
...
// TestResolve_GroupLongContextDisabledCollapsesChannelIntervals pins the
// checkbox semantics for channel token ladders: 关掉后只保留最低档。
func TestResolve_GroupLongContextDisabledCollapsesChannelIntervals(t *testing.T) {
	r := newResolverWithChannel(t, []ChannelModelPricing{{
		Platform: "anthropic", Models: []string{"claude-sonnet-4"}, BillingMode: BillingModeToken,
		Intervals: []PricingInterval{
			{MinTokens: 200000, InputPrice: testPtrFloat64(18e-6)},
	...
	require.Equal(t, PricingSourceChannel, resolved.Source)
	require.Empty(t, resolved.Intervals)
	require.InDelta(t, 9e-6, r.GetIntervalPricing(resolved, 300000).InputPricePerToken, 1e-12)
...
// TestResolve_NilGroupKeepsLongContextLadders 保证未传分组时官方阶梯照常生效。
func TestResolve_NilGroupKeepsLongContextLadders(t *testing.T) {
	bs := newTestBillingServiceForResolver()
	r := NewModelPricingResolver(nil, bs)
