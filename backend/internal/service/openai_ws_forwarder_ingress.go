...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	if forceHTTPBridge || s.shouldBridgeOpenAIWSHTTP(account, firstPayload.payloadBytes, firstPayload.previousResponseID) {
		...
		for turn := 1; ; turn++ {
			...
			if bridgeErr != nil {
				// Turn 1 failover is handled by the caller replaying the first
				// message. From turn 2 the first message alone would drop the
				// conversation, so hand the rebuilt payload up with the error.
