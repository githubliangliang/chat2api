...
func RunCLI() error {
	...
	fmt.Println()
	fmt.Println("── Redis Configuration (optional) ──")
	fmt.Println("  Redis is optional. Disable to use embedded in-process Redis (single-node only).")

	useRedis := promptConfirm(reader, "Use external Redis?")
	cfg.Redis.Enabled = boolPtr(useRedis)
	if useRedis {
		for {
		...
		cfg.Redis.Host = ""
		fmt.Println("  Redis disabled — embedded Redis will be used at runtime.")
	}
