...
func RegisterUserRoutes(
	...
	authenticated.Use(gin.HandlerFunc(auditLog))
	{
		...
		// 页面已下线的能力：保留最小 stub，避免前端轮询刷 404/红字。
		announcements := authenticated.Group("/announcements")
		{
			announcements.GET("", func(c *gin.Context) {
				c.JSON(200, gin.H{"code": 0, "message": "success", "data": []any{}})
			})
			announcements.POST("/:id/read", func(c *gin.Context) {
		...
		subscriptions := authenticated.Group("/subscriptions")
		{
			...
			subscriptions.GET("", empty)
			subscriptions.GET("/active", empty)
			subscriptions.GET("/progress", emptyObj)
			subscriptions.GET("/summary", emptyObj)
		}
	}
}
