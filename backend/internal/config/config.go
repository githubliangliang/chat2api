...
func (d *DatabaseConfig) IsSQLite() bool {
	return d.NormalizedDriver() == DatabaseDriverSQLite
}

// SQLitePath returns the SQLite file path with a sensible default.
...
// Redis 为可选组件：enabled=false 时使用进程内嵌入式 Redis（单机模式）。
...
// IsEnabled reports whether an external Redis is required.
// When false, the application uses an embedded in-process Redis (single-node only).
func (r *RedisConfig) IsEnabled() bool {
	if r.Enabled != nil {
		return *r.Enabled
	}
	// Backward compatible default: enabled unless host is explicitly empty.
...
func setDefaults() {
	...
	// Redis（可选：enabled=false 时使用进程内嵌入式 Redis）
...
func (c *Config) Validate() error {
	...
	if c.Database.IsSQLite() {
		if strings.TrimSpace(c.Database.SQLitePath()) == "" {
			return fmt.Errorf("database.path must not be empty when database.driver=sqlite")
		}
		// SQLite benefits from a small pool (often 1 writer); keep validation light.
	...
	// Redis is optional. When disabled we only still need sane pool defaults for the embedded client.
	...
	if c.Redis.IsEnabled() && strings.TrimSpace(c.Redis.Host) == "" {
		return fmt.Errorf("redis.host must not be empty when redis.enabled=true")
	}
