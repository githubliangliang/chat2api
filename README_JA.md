...
初期管理者アカウントはセットアップウィザード、または空のデータベースでの初回起動時に `config.yaml` の `default.admin_email` / `default.admin_password` から作成できます。既存ユーザーのパスワードは上書きされません。

設定テンプレートを事前に配置してそのまま起動できますが、公開サンプル資格情報を防ぐため、`admin123` と `CHANGE_ME_STRONG_PASSWORD` は拒否されます。実際の強力なパスワードに変更してください。両フィールドを設定しない場合は `http://<host>:8080/setup` でウィザードを完了します。
