...
// User-facing nav (regular sidebar + admin "My Account"). /dashboard visibility
// is controlled by hidden_menu_keys, not by caller.
...
// finalizeNav 合并多重过滤：featureFlag + simple 模式 + 管理员配置的隐藏菜单。
function finalizeNav(items: NavItem[]): NavItem[] {
  let visible = applyFeatureFlags(items)
  if (authStore.isSimpleMode) {
    visible = visible.filter((item) => !item.hideInSimpleMode)
  }

  // Public settings (SSR/API) + admin store (fresher after menu management save).
  const hidden = new Set<string>()
  for (const raw of appStore.cachedPublicSettings?.hidden_menu_keys ?? []) {
    const key = String(raw || '').trim()
    if (key) hidden.add(key)
  }
  for (const raw of adminSettingsStore.hiddenMenuKeys ?? []) {
    const key = String(raw || '').trim()
    if (key) hidden.add(key)
  }
  ...
  const filterHidden = (list: NavItem[]): NavItem[] => {
    const out: NavItem[] = []
    for (const item of list) {
      if (hidden.has(item.path)) continue
      if (item.children?.length) {
        const children = filterHidden(item.children)
        // Drop expand-only parents that have no remaining children.
  ...
  return filterHidden(visible)
}

// User navigation items (for regular users)
const userNavItems = computed((): NavItem[] => finalizeNav(buildSelfNavItems()))

// Personal navigation items (admin "My Account"). Same catalog as regular users,
// including /dashboard — /admin/dashboard was removed, so this is the only entry.
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/model-plaza', label: t('nav.modelPlaza'), icon: DashboardIcon },
  ...
  // Simple mode hides the personal section, so surface toggleable user menus here.
  if (authStore.isSimpleMode) {
    const filtered = visible.filter(item => !item.hideInSimpleMode)
    filtered.push({ path: '/dashboard', label: t('nav.dashboard'), icon: DashboardIcon })
    filtered.push({ path: '/keys', label: t('nav.apiKeys'), icon: KeyIcon })
    filtered.push({ path: '/admin/menu', label: t('nav.menuManagement'), icon: MenuIcon })
    filtered.push({ path: '/admin/settings', label: t('nav.settings'), icon: CogIcon })
    for (const cm of customMenuItemsForAdmin.value) {
      filtered.push({ path: `/custom/${cm.id}`, label: cm.label, icon: null, iconSvg: cm.icon_svg })
    }
    return finalizeNav(filtered)
  }

  visible.push({ path: '/admin/menu', label: t('nav.menuManagement'), icon: MenuIcon })
  visible.push({ path: '/admin/settings', label: t('nav.settings'), icon: CogIcon })
  for (const cm of customMenuItemsForAdmin.value) {
    visible.push({ path: `/custom/${cm.id}`, label: cm.label, icon: null, iconSvg: cm.icon_svg })
  }
  return finalizeNav(visible)
})
